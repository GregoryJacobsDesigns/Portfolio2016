		...
		public function render_benefit( $attributes, $content = null ) {
			...
			if ( $attributes['icon_code'] ) {
				...
				if ( !empty($default_image) ) {
					...
					// @GJ Commented Out and changed to <embed> for purpose of using SVG's
					// $image = sprintf( '<img src="%s" %s alt="%s" />', $image, $image_size, esc_attr( $attributes['image_alt'] ) );
					$image = sprintf( '<object><embed src="%s" %s alt="%s"/></object>', $image, $image_size, esc_attr( $attributes['image_alt'] ) );
				}
			}
